function out = mzxmlread(filename,varargin)
...
%
%   MZXMLREAD(...,'SCANINDICES',INDICES) specifies an index, a vector of
...
%   the scan number, as noted by num attribute in the scan element, with
%   which to start extracting scan information.  END_IND is an integer
%   indicating the scan number with which to stop extracting scan
%   information. START_IND must be less then END_IND.  Default is to
...
if out.scan(1).num == 0
    for i=1:length(out.scan)
        out.scan(i).num = out.scan(i).num+1;
    end
end
clear i;
    ...
    switch(k)
        ...
        case 2  % timerange
            ...
            elseif ~strcmp(mzinfo.StartTime,'N/A') && ~strcmp(mzinfo.EndTime,'N/A')
                ...
                if pval(1)<startTm || pval(2)>endTm
                    error(message('bioinfo:mzxmlread:InvalidTimeRange', startTm, endTm));                    
                end
            end
            tmrange = pval;                
        ...
        case 3  % scanindices
            ...
            elseif ~strcmp(mzinfo.NumberOfScans,'N/A')                                  
               if pval(1)<0 || pval(2)>mzinfo.NumberOfScans
